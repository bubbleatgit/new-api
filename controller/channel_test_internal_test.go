...
func TestShouldEnableAfterTest(t *testing.T) {
	...
	someErr := relaykittypes.NewOpenAIError(
		fmt.Errorf("boom"),
		relaykittypes.ErrorCodeBadResponse,
	...
	cases := []struct {
		...
		{
			name:          "multi-key auto disabled channel key success -> recover",
			enableSwitch:  true,
			forceKeyIndex: 0,
			isChannelOn:   false,
		...
		{
			name:          "multi-key empty using key -> no recover",
			enableSwitch:  true,
			forceKeyIndex: 0,
			isChannelOn:   true,
			channelStatus: common.ChannelStatusEnabled,
		...
		{
			name:          "single-key auto disabled failed probe -> no recover",
			enableSwitch:  true,
			forceKeyIndex: -1,
			isChannelOn:   false,
			channelStatus: common.ChannelStatusAutoDisabled,
		...
		{
			name:          "single-key enabled channel success -> no recover",
			enableSwitch:  true,
			forceKeyIndex: -1,
			isChannelOn:   true,
			channelStatus: common.ChannelStatusEnabled,
